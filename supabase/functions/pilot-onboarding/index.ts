const ORIGINS=['https://haqn455-coder.github.io'];
const PUB='sb_publishable_3G6sjO0orxo7FjoS2hUNDw_bbsavIbw';
function appBase(origin:string){return origin==='https://haqn455-coder.github.io'?origin+'/phc-compliance-os-ui/':origin+'/'}
function tenantRedirect(origin:string,tenantId:string,pilotCode:string){const u=new URL('clinic.html',appBase(origin));u.searchParams.set('tenant',tenantId);u.searchParams.set('pilot',pilotCode);return u.toString()}
function allowedOrigin(req:Request){const o=req.headers.get('origin')||'';return ORIGINS.includes(o)?o:''}
function cors(req:Request){const o=allowedOrigin(req);return {'access-control-allow-origin':o||'null','access-control-allow-headers':'authorization,content-type,apikey,x-client-info','access-control-allow-methods':'GET,POST,OPTIONS','vary':'Origin'}}
...
async function magicLink(req:Request,admin:any,email:string,tenantId:string,pilotCode:string){return admin.auth.admin.generateLink({type:'magiclink',email,options:{redirectTo:tenantRedirect(allowedOrigin(req),tenantId,pilotCode)}})}
...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  if(p==='/health')return json(req,{ok:true,version:6,service:'pilot-onboarding',origins:ORIGINS});
  if(p==='/request'&&req.method==='POST'){
   const b=await req.json();if(clean(b.website))return json(req,{ok:true});
   const clinic_name=clean(b.clinic_name,120),city=clean(b.city,80),district=clean(b.district,80),owner_name=clean(b.owner_name,120),owner_email=clean(b.owner_email,180).toLowerCase(),phone=clean(b.phone,40),clinic_type=clean(b.clinic_type,40);
   ...
   const ins=await admin.from('pilot_requests').insert({clinic_name,city,district:district||null,owner_name,owner_email,phone:phone||null,clinic_type,phc_status:'UNKNOWN',terms_accepted_at:new Date().toISOString(),commercial_status:'TRIAL_ACTIVE',plan_code:'TRIAL_3D'}).select('id,request_ref,status,submitted_at').single();
  ...
  if(p==='/session/open'&&req.method==='POST'){
   ...
   const gate=await admin.rpc('open_pilot_entitlement',{p_tenant_id:tid,p_user_id:a.user.id});
   if(gate.error)return json(req,{error:'pilot entitlement could not be verified',code:'ENTITLEMENT_CHECK_FAILED'},500);
   if(!gate.data?.allowed)return json(req,{error:gate.data?.message||'Clinic access is unavailable.',code:gate.data?.code||'ENTITLEMENT_DENIED',entitlement:gate.data},403);
   const fwd=(req.headers.get('x-forwarded-for')||'').split(',')[0].trim(),iph=fwd?await sha(fwd):null;
   await admin.from('pilot_access_events').insert({pilot_request_id:gate.data.pilot_request_id,tenant_id:tid,user_id:a.user.id,event_type:'WORKSPACE_OPEN',device_id:device_id||null,user_agent:ua||null,client_timezone:tz||null,client_language:lang||null,ip_hash:iph});
   return json(req,{ok:true,tenant_id:tid,role:membership.role,pilot_code:gate.data.pilot_code||null,status:gate.data.commercial_status,clinic_name:gate.data.clinic_name||null,entitlement:gate.data});
  }
  ...
  if(p==='/admin/list'&&req.method==='GET'){const q=await admin.from('pilot_requests').select('id,request_ref,status,clinic_name,city,district,owner_name,owner_email,phone,clinic_type,phc_status,services,staff_summary,operating_hours,google_account_available,submitted_at,pilot_code,tenant_id,facility_id,register_workbook_url,drive_folder_url,verification_result,verified_at,activated_at,pilot_start_date,pilot_end_date,commercial_status,plan_code,trial_started_at,trial_expires_at,paid_until_at,updated_at').order('submitted_at',{ascending:false}).limit(100);if(q.error)return json(req,{error:q.error.message},400);return json(req,{ok:true,requests:q.data})}
  ...
  if(p==='/admin/access-link'&&req.method==='POST'){const b=await req.json(),pr=await admin.from('pilot_requests').select('owner_email,pilot_code,status,commercial_status,tenant_id').eq('id',clean(b.id,80)).single();if(pr.error)return json(req,{error:pr.error.message},400);if(['SUSPENDED','CLOSED'].includes(pr.data.status)||pr.data.commercial_status==='SUSPENDED')return json(req,{error:'Pilot access is suspended/closed'},409);const link=await magicLink(req,admin,pr.data.owner_email,pr.data.tenant_id,pr.data.pilot_code);if(link.error)return json(req,{error:link.error.message},400);return json(req,{ok:true,owner_access_link:link.data.properties?.action_link||null,pilot_code:pr.data.pilot_code})}
  if(p==='/admin/entitlement'&&req.method==='POST'){const b=await req.json(),r=await admin.rpc('set_pilot_entitlement',{p_request_id:clean(b.id,80),p_actor_id:a.user.id,p_action:clean(b.action,40),p_reason:clean(b.reason,500)||null});if(r.error)return json(req,{error:r.error.message},400);return json(req,{ok:true,result:r.data})}
  if(p==='/admin/suspend'&&req.method==='POST'){const b=await req.json(),r=await admin.rpc('set_pilot_entitlement',{p_request_id:clean(b.id,80),p_actor_id:a.user.id,p_action:'SUSPEND',p_reason:clean(b.reason,500)||null});if(r.error)return json(req,{error:r.error.message},400);return json(req,{ok:true,result:r.data})}
  if(p==='/admin/resume'&&req.method==='POST'){const b=await req.json(),r=await admin.rpc('set_pilot_entitlement',{p_request_id:clean(b.id,80),p_actor_id:a.user.id,p_action:'RESUME',p_reason:null});if(r.error)return json(req,{error:r.error.message},400);return json(req,{ok:true,result:r.data})}
