      ...
      <div className="mt-4 space-y-3">{visible.map(r=>{
        ...
        return <article key={r.id} className="overflow-hidden rounded-2xl border">
          <button onClick={()=>setOpen(x=>({...x,[r.indicator_no]:!x[r.indicator_no]}))} className="flex w-full items-start gap-3 bg-white p-4 text-left">
            ...
            <span className="flex-1"><span className="block text-xs font-bold text-slate-500">{r.standard_code}</span><span className="block font-bold">{r.indicator_text}</span><span className="mt-2 flex flex-wrap gap-1"><span className={cx('rounded-full border px-2 py-1 text-[11px] font-black',stateTone(r.state))}>{r.state.replace(/_/g,' ')}</span><span className="rounded-full border bg-slate-50 px-2 py-1 text-[11px] font-bold">Review: {r.review_decision||'PENDING'}</span><span className="rounded-full border bg-slate-50 px-2 py-1 text-[11px] font-bold">Evidence: {total}</span></span></span><span className="rounded-lg bg-slate-100 px-2 py-1 font-black">{open[r.indicator_no]?'⌃':'⌄'}</span>
          </button>
